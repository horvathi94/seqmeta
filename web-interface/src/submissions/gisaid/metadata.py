...
from seqmeta.submissions.excel import ExcelFile
...
class Metadata:
    ...
    def create_header(self, sample: "Sample") -> None:
        for index, a in enumerate(sample.list_gisaid_attributes()):
...
#        self.append_assembly(sample)
    ...
    def from_samples(self, samples: List["Sample"]) -> None:
        self.create_header(samples[0])
        self.assemblies_filename = samples[0].get_value("gisaid_filename")
        for s in samples:
            self.add_sample(s)
