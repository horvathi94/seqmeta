...
`;function appendInfo(E,T){E=E.charAt(0).toUpperCase()+E.slice(1);const L=document.querySelector("#ena-checklist-info"),A=document.createElement("dt"),U=document.createElement("dd");A.innerHTML=E,U.innerHTML=T,L.appendChild(A),L.appendChild(U)}function changedKeep(E){var T=E.getAttribute("name").split("+")[1];const L=document.getElementsByName(`inpfield+${T}+keep`)[0];E.checked?L.value=1:L.value=0}function replaceIndices(E,T){Array.from(E.children).forEach(E=>{Array.from(E.children).forEach(E=>{E.setAttribute("name",E.name.replace("+0+",`+${T}+`))})})}function createDefaultValSelect(E){const L=document.createElement("select"),T=document.createElement("option");return T.innerHTML="---------",T.value="",L.add(T),E.options.forEach(E=>{let T=document.createElement("option");T.innerHTML=E,T.value=E,L.add(T)}),L}function replaceDefaultValField(E,T,L){let A=E.querySelector(`[name='inpfield+${T}+default']`),U=null;"select"==L.type?U=createDefaultValSelect(L):"text"==L.type&&null!==L.regex?A.setAttribute("pattern",L.regex):"textarea"==L.type&&(U=document.createElement("textarea")),null!==U&&(U.setAttribute("name",A.getAttribute("name")),A.parentNode.replaceChild(U,A))}function setRowValues(E,T,L){E.querySelector(`[name='inpfield+${T}+name']`).value=L.name,E.querySelector(`[name='inpfield+${T}+description']`).value=L.description,E.querySelector(`[name='inpfield+${T}+importance']`).value=L.importance;const A=E.querySelector(`[name='ommit+${T}+pseudokeep']`),U=E.querySelector(`[name='inpfield+${T}+keep']`);"mandatory"==L.importance?(U.value=1,A.disabled=!0,A.checked=!0):"recommended"==L.importance&&(U.value=1,A.checked=!0),replaceDefaultValField(E,T,L.default)}function addNewRow(E=null){const T=document.querySelector("#fields-editor-tab").tBodies[0],L=T.querySelector("tr.template").cloneNode(!0);var A=T.rows.length-1;L.classList.remove("template"),replaceIndices(L,A),T.appendChild(L),null!==E&&setRowValues(L,A,E)}function parseDefaultValue(E){E=E.querySelector(":scope > FIELD_TYPE").children[0];let T={type:"text",regex:null,options:[]};return"TEXT_CHOICE_FIELD"==E.tagName?(T.type="select",T.options=Array.from(E.children).map(E=>E.children[0].innerHTML)):"TEXT_AREA_FIELD"==E.tagName?T.type="textarea":"TEXT_FIELD"==E.tagName&&0<E.children.length&&"REGEX_VALUE"==E.children[0].tagName&&(T.regex=E.children[0].innerHTML),T}function parseAttr(E){return{name:E.querySelector(":scope > NAME").innerHTML,importance:E.querySelector(":scope > MANDATORY").innerHTML,description:E.querySelector(":scope > DESCRIPTION").innerHTML,default:parseDefaultValue(E)}}function handleChecklist(E,T){var T=`/CHECKLIST_SET/CHECKLIST[@checklistType='Sample' and @accession='${T}']`,T=(appendInfo("primary id",E.evaluate(T+"/IDENTIFIERS/PRIMARY_ID",E,null,XPathResult.STRING_TYPE,null).stringValue),T+"/DESCRIPTOR"),L=(appendInfo("label",E.evaluate(T+"/LABEL",E,null,XPathResult.STRING_TYPE,null).stringValue),E.evaluate(T+"/NAME",E,null,XPathResult.STRING_TYPE,null).stringValue),L=(appendInfo("name",L),E.evaluate(T+"/DESCRIPTION",E,null,XPathResult.STRING_TYPE,null).stringValue),L=(appendInfo("name",L),E.evaluate(T+"/AUTHORITY",E,null,XPathResult.STRING_TYPE,null).stringValue),T=(appendInfo("authority",L),E.getElementsByTagName("FIELD_GROUP"));let A,U,V;Array.from(T).forEach(E=>{A=null===(A=E.querySelector(":scope > NAME"))?"N/A":A.innerHTML,U=null===(U=E.querySelector(":scope > DESCRIPTION"))?"N/A":U.innerHTML,appendInfo(A,U),V=E.querySelectorAll(":scope > FIELD"),Array.from(V).forEach(E=>{addNewRow(parseAttr(E))})})}function fetchXML(){const E=document.querySelector("#ena-checklist-accession").value;handleChecklist(xml=(new window.DOMParser).parseFromString(rawXML,"text/xml"),E)}fetchXML();