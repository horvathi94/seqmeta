...
`;function appendInfo(E,T){E=E.charAt(0).toUpperCase()+E.slice(1);const L=document.querySelector("#ena-checklist-info"),A=document.createElement("dt"),U=document.createElement("dd");A.innerHTML=E,U.innerHTML=T,L.appendChild(A),L.appendChild(U)}function levelHidden(E,T){const L=document.createElement("input");return L.setAttribute("type","hidden"),"mandatory"==E?L.value=3:"recommended"==E?L.value=2:"optional"==E&&(L.value=1),L.setAttribute("name",T),L}function changedKeep(E){var T=E.getAttribute("name").split("+")[1];const L=document.getElementsByName(`inpfield+${T}+keep`)[0];E.checked?L.value=1:L.value=0}function keepCheckbox(E,T){const L=document.createElement("input");return L.setAttribute("type","checkbox"),L.setAttribute("name",T),"mandatory"==E?(L.setAttribute("checked",!0),L.setAttribute("disabled",!0)):"recommended"==E&&L.setAttribute("checked",!0),L.setAttribute("onchange","changedKeepCheckbox(this);"),L}function keepCheckboxVal(E,T){const L=document.createElement("input");return L.setAttribute("type","hidden"),L.setAttribute("name",T),L.value="mandatory"==E||"recommended"==E?1:0,L}function descriptionTextArea(E,T){const L=document.createElement("textarea");return L.innerHTML=E,L.setAttribute("name",T),L}function createDefaultValSelect(E){const L=document.createElement("select");let A;const T=document.createElement("option");return T.innerHTML="---------",T.value="",L.add(T),Array.from(E.children).forEach(E=>{let T=document.createElement("option");A=E.children[0].innerHTML,T.innerHTML=A,T.value=A,L.add(T)}),L}function createDefaultValTextArea(E){return document.createElement("textarea")}function createDefaultValText(E,T){const L=document.createElement("input");return L.setAttribute("type","text"),null!==T&&L.setAttribute("pattern",T),L}function defaultValue(T,E){T=T.children[0];let L;if("TEXT_CHOICE_FIELD"==T.tagName)L=createDefaultValSelect(T);else if("TEXT_AREA_FIELD"==T.tagName)L=createDefaultValTextArea(T);else if("TEXT_FIELD"==T.tagName){let E=null;0<T.children.length&&"REGEX_VALUE"==T.children[0].tagName&&(E=T.children[0].innerHTML),L=createDefaultValText(T,E)}return L.setAttribute("name",E),L}function nameField(E,T,L=!0){const A=document.createElement("input");return A.setAttribute("type","text"),A.setAttribute("name",T),A.value=E,L&&A.setAttribute("readonly",!0),A}function addNewRowOld(E,T){const L=document.querySelector("#fields-editor-tab");row=L.insertRow();var A="inpfield+"+String(T)+"+";const U=row.insertCell(0);var V=E.querySelector(":scope > MANDATORY").innerHTML;U.appendChild(levelHidden(V,A+"level")),U.appendChild(keepCheckbox(V,"ommit+"+String(T)+"+pseudokeep")),U.appendChild(keepCheckboxVal(V,A+"keep"));const e=row.insertCell(1);T=E.querySelector(":scope > NAME").innerHTML,e.appendChild(nameField(T,A+"name")),V=E.querySelector(":scope > FIELD_TYPE");const a=row.insertCell(2),n=(a.appendChild(defaultValue(V,A+"default")),row.insertCell(3));T=E.querySelector(":scope > DESCRIPTION").innerHTML;n.appendChild(descriptionTextArea(T,A+"description"))}function replaceIndices(E,T){Array.from(E.children).forEach(E=>{Array.from(E.children).forEach(E=>{E.setAttribute("name",E.name.replace("+0+",`+${T}+`))})})}function addNewRow(E=null){const T=document.querySelector("#fields-editor-tab").tBodies[0],L=T.querySelector("tr.template").cloneNode(!0);var A=T.rows.length-1;L.classList.remove("template"),replaceIndices(L,A),T.appendChild(L),console.dir(L),null!==E&&((cell=L.querySelector(`input[name='inpfield+${A}+name]'`)).value=E.name,console.log(cel))}function addField(E){var T=E.querySelector(":scope > LABEL").innerHTML,L=E.querySelector(":scope > NAME").innerHTML,A=E.querySelector(":scope > DESCRIPTION").innerHTML,E=(E.querySelector(":scope > FIELD_TYPE"),E.querySelector(":scope > MANDATORY").innerHTML),T=createField(T,L,E,A);fieldEditor.appendChild(T)}function createAttr(E){return{name:E.querySelector(":scope > NAME").innerHTML}}function handleChecklist(E,T){var T=`/CHECKLIST_SET/CHECKLIST[@checklistType='Sample' and @accession='${T}']`,T=(appendInfo("primary id",E.evaluate(T+"/IDENTIFIERS/PRIMARY_ID",E,null,XPathResult.STRING_TYPE,null).stringValue),T+"/DESCRIPTOR"),L=(appendInfo("label",E.evaluate(T+"/LABEL",E,null,XPathResult.STRING_TYPE,null).stringValue),E.evaluate(T+"/NAME",E,null,XPathResult.STRING_TYPE,null).stringValue),L=(appendInfo("name",L),E.evaluate(T+"/DESCRIPTION",E,null,XPathResult.STRING_TYPE,null).stringValue),L=(appendInfo("name",L),E.evaluate(T+"/AUTHORITY",E,null,XPathResult.STRING_TYPE,null).stringValue),T=(appendInfo("authority",L),E.getElementsByTagName("FIELD_GROUP"));let A,U,V;Array.from(T).forEach(E=>{A=null===(A=E.querySelector(":scope > NAME"))?"N/A":A.innerHTML,U=null===(U=E.querySelector(":scope > DESCRIPTION"))?"N/A":U.innerHTML,appendInfo(A,U),V=E.querySelectorAll(":scope > FIELD"),Array.from(V).forEach(E=>{addNewRow(createAttr(E))})})}function fetchXML(){const E=document.querySelector("#ena-checklist-accession").value;handleChecklist(xml=(new window.DOMParser).parseFromString(rawXML,"text/xml"),E)}fetchXML();