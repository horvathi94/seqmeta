...
`;function appendInfo(E,T){E=E.charAt(0).toUpperCase()+E.slice(1);const L=document.querySelector("#ena-checklist-info"),A=document.createElement("dt"),U=document.createElement("dd");A.innerHTML=E,U.innerHTML=T,L.appendChild(A),L.appendChild(U)}function determineType(E){const T=E.children[0];if("TEXT_FIELD"==T.tagName)return E=T.querySelector("REGEX_VALUE"),console.log(E),"text"}function addField(E){const T=document.querySelector("#fields-editor");var L=E.querySelector(":scope > LABEL").innerHTML;const A=E.querySelector(":scope > NAME").innerHTML;E.querySelector(":scope > DESCRIPTION").innerHTML;var U=E.querySelector(":scope > FIELD_TYPE"),E=E.querySelector(":scope > MANDATORY").innerHTML;document.createElement("div.field");let V=document.createElement("input");if("TEXT_FIELD"==U.children[0].tagName){V.setAttribute("type","text");var e=U.children[0].children;0<e.length&&"REGEX_VALUE"==e[0].tagName&&V.setAttribute("pattern",e[0].innerHTML)}else if("TEXT_CHOICE_FIELD"==U.children[0].tagName){V=document.createElement("select");let T;Array.from(U.children[0].children).forEach(E=>{value=E.querySelector("VALUE").innerHTML,(T=document.createElement("option")).innerHTML=value,T.value=value,V.appendChild(T)})}const a=document.createElement("label"),_=(a.innerHTML=L+` (${E}):`,V.setAttribute("name",A.replace(" ","_")),document.createElement("div"));_.style.display="block",_.appendChild(a),_.appendChild(V),T.append(_)}function handleChecklist(E,T){var T=`/CHECKLIST_SET/CHECKLIST[@checklistType='Sample' and @accession='${T}']`,T=(appendInfo("primary id",E.evaluate(T+"/IDENTIFIERS/PRIMARY_ID",E,null,XPathResult.STRING_TYPE,null).stringValue),T+"/DESCRIPTOR"),L=(appendInfo("label",E.evaluate(T+"/LABEL",E,null,XPathResult.STRING_TYPE,null).stringValue),E.evaluate(T+"/NAME",E,null,XPathResult.STRING_TYPE,null).stringValue),L=(appendInfo("name",L),E.evaluate(T+"/DESCRIPTION",E,null,XPathResult.STRING_TYPE,null).stringValue),L=(appendInfo("name",L),E.evaluate(T+"/AUTHORITY",E,null,XPathResult.STRING_TYPE,null).stringValue),T=(appendInfo("authority",L),E.getElementsByTagName("FIELD_GROUP"));let A,U,V;Array.from(T).forEach(E=>{A=null===(A=E.querySelector(":scope > NAME"))?"N/A":A.innerHTML,U=null===(U=E.querySelector(":scope > DESCRIPTION"))?"N/A":U.innerHTML,appendInfo(A,U),V=E.querySelectorAll(":scope > FIELD"),Array.from(V).forEach(E=>{addField(E)})})}function fetchXML(){const E=document.querySelector("#ena-checklist-accession").value;handleChecklist(xml=(new window.DOMParser).parseFromString(rawXML,"text/xml"),E)}fetchXML();